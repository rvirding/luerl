...
define_fun_in_lua_test() ->
    ...
    {[Fun2], State2} = luerl:call_function([mkadder], [1], State1),
    {[Fun3], State3} = luerl:call_function([mkadder], [2], State1),
...
define_fun2_in_lua_test() ->
    ...
    Chunk = <<"function mklist(numentries)\n"
              ...
              "end\n">>,
    {_, State1} = luerl:do(Chunk, State),
    {[Fun2], State2} = luerl:call_function1([<<"mklist">>], [5], State1),

    %% Must decode the return values as they are tables.
    {Res21,State21} = luerl:call_function1(Fun2, [4], State2),
    ?assertMatch([[{1,4}, {2,4}, {3,4}, {4,4}, {5,4}]],
                 luerl:decode_list(Res21, State21)),
    {Res22,State22} = luerl:call_function1(Fun2, [4.0], State2),
    ?assertMatch([[{1,4.0}, {2,4.0}, {3,4.0}, {4,4.0}, {5,4.0}]],
                 luerl:decode_list(Res22, State22)),

    {[Fun3], State3} = luerl:call_function([<<"mklist">>], [10], State1),
