...
-module(luerl_old_funcall_tests).
...
external_fun_test() ->
    ...
    State1 = luerl_old:set_table([<<"testFun">>], F, State),
    {_, State2} = luerl_old:do(
        <<"function test(i)\n  local a, b = testFun(i)\n return (a == i + 2), (b[1] == i + 3), (b[2] == i + 4) end">>,
        State1
    ),
    {Res, _State3} = luerl_old:call_function([test], [2], State2),
...
external_nostate_fun_test() ->
    ...
    Chunk = <<
        ...
        "end"
    >>,
    {_, State2} = luerl_old:do(Chunk, State1),
    {Res, _State3} = luerl_old:call_function([test], [2], State2),
    [BoolVal, BoolVal2, BoolVal3] = Res,
    ?assertEqual(true, BoolVal),
    ?assertEqual(true, BoolVal2),
...
return_lib_function_test() ->
    State = luerl_old:init(),
    {_, State1} = luerl_old:do(<<"function test()\n  return string.find  end\n">>, State),
    {[{M, F, A}], _State2} = luerl_old:call_function([test], [1], State1),
...
define_fun_in_lua_test() ->
    ...
    Chunk = <<
        ...
        "end\n"
    >>,
    {_, State1} = luerl_old:do(Chunk, State),
    {[Fun], _State2} = luerl_old:call_function([mkadder], [1], State1),
    {[Fun2], _State3} = luerl_old:call_function([mkadder], [2], State1),
    ?assertEqual([5], Fun([4])),
...
define_fun2_in_lua_test() ->
    ...
    >>,
    {_, State1} = luerl_old:do(Chunk, State),
    {[Fun], _State2} = luerl_old:call_function([mklist], [5], State1),
    {[Fun2], _State3} = luerl_old:call_function([mklist], [10], State1),
    ?assertEqual(
        [[{1, 4}, {2, 4}, {3, 4}, {4, 4}, {5, 4}]],
        Fun([4])
    ),
...
newindex_metamethod_test() ->
    ...
    Chunk = <<
        ...
        "return t[123], m[123]"
    >>,
    {[TVal, MVal], _State1} = luerl_old:do(Chunk, State),
