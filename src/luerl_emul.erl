...
-define(ITRACE_DO(Expr), begin
    (get(luerl_itrace) /= undefined) andalso Expr
end).
...
init() ->
    ...
    %Global environment
    {_G, St2} = luerl_lib_basic:install(St1),
    St3 = St2#luerl{g = _G},
...
get_local_var(D, I, [_ | Fs]) -> get_local_var(D - 1, I, Fs).
...
%This is dynamic
load_chunk_i(?REPEAT_LOOP(B0), Funrs0, St0) ->
    {B1, Funrs1, St1} = load_chunk_is(B0, Funrs0, St0),
...
%This is dynamic
load_chunk_i(?NFOR_LOOP(N, L, S, B0), Funrs0, St0) ->
    {B1, Funrs1, St1} = load_chunk_is(B0, Funrs0, St0),
...
%This is dynamic
load_chunk_i(?GFOR_CALL(F, D, V, B0), Funrs0, St0) ->
    {B1, Funrs1, St1} = load_chunk_is(B0, Funrs0, St0),
    {?GFOR_CALL(F, D, V, B1), Funrs1, St1};
%This is dynamic
load_chunk_i(?GFOR_LOOP(F, D, B0), Funrs0, St0) ->
    {B1, Funrs1, St1} = load_chunk_is(B0, Funrs0, St0),
    {?GFOR_LOOP(F, D, B1), Funrs1, St1};
%% Then the rest which we don't have to worry about.
load_chunk_i(I, Funrs, St) ->
    {I, Funrs, St}.
...
%Lua function
call(#funref{} = Funref, Args, St0) ->
    {Ret, St1} = functioncall(Funref, Args, St0),
    %% Should do GC here.
    {Ret, St1};
%Erlang function
call(#erl_func{} = Func, Args, St0) ->
    {Ret, St1} = functioncall(Func, Args, St0),
    %% Should do GC here.
    {Ret, St1};
%Erlang function as MFA triplet
call(#erl_mfa{} = Func, Args, St0) ->
    {Ret, St1} = functioncall(Func, Args, St0),
    {Ret, St1}.
...
emul_1([?PUSH_GVAR(Key) | Is], Cont, Lvs, Stk, Env, Cs, St0) ->
    ...
    case luerl_heap:get_global_key(Key, St0) of
        {value, Val, St1} ->
            emul(Is, Cont, Lvs, [Val | Stk], Env, Cs, St1);
        {meta, Meth, Args, St1} ->
...
emul_1([?PUSH_LAST_GVAR(Key) | Is], Cont, Lvs, Stk, Env, Cs, St0) ->
    ...
    case luerl_heap:get_global_key(Key, St0) of
        ...
        {meta, Meth, Args, St1} -> emul([?FCALL | Is], Cont, Lvs, [Args, Meth | Stk], Env, Cs, St1);
        {error, Error, St1} -> lua_error(Error, St1#luerl{stk = Stk, cs = Cs})
    end;
...
pop_vals(0, Stk) -> {[], Stk};
%This a list tail
pop_vals(C, [Vtail | Stk]) -> pop_vals(C - 1, Stk, Vtail).

pop_vals(0, Stk, Vs) -> {Vs, Stk};
pop_vals(1, [V | Stk], Vs) -> {[V | Vs], Stk};
pop_vals(2, [V2, V1 | Stk], Vs) -> {[V1, V2 | Vs], Stk};
pop_vals(C, [V2, V1 | Stk], Vs) -> pop_vals(C - 2, Stk, [V1, V2 | Vs]).
...
push_vals(C, [V | Vs], Stk) -> push_vals(C - 1, Vs, [V | Stk]);
push_vals(C, [], Stk) -> push_vals(C - 1, [], [nil | Stk]).
...
do_op1(Is, Cont, Lvs, [A | Stk], Env, Cs, St0, Op) ->
    ...
    case op(Op, A, St0) of
        {value, Res, St1} ->
            emul(Is, Cont, Lvs, [Res | Stk], Env, Cs, St1);
        {meta, Meth, Args, St1} ->
            emul([?FCALL, ?SINGLE | Is], Cont, Lvs, [Args, Meth | Stk], Env, Cs, St1);
        {error, Error, St1} ->
            lua_error(Error, St1#luerl{stk = Stk, cs = Cs})
...
do_return(Ac, Stk0, Cs0, St0) ->
    ...
    {Cf, Cs1} = find_call_frame(Cs0, St0),
...
%Skip current line info
find_loop_frame([#current_line{} | Cs], St) ->
    find_loop_frame(Cs, St);
find_loop_frame([#loop_frame{} = Bf | Cs], _St) ->
    {Bf, Cs};
find_loop_frame(Cs, St) ->
...
do_current_line(Is, Cont, Lvs, Stk, Env, Cs0, St0, Line, File) ->
    %Push onto callstack
    Cs1 = push_current_line(Cs0, Line, File),
    %% When tracing bring the state up to date and call the tracer.
    Tfunc = St0#luerl.trace_func,
    St1 =
        if
...
call_luafunc(
    #lua_func{lsz = Lsz, esz = Esz, pars = _Pars, body = Fis},
    Args,
    Stk0,
    Env0,
    Cs,
...
make_env_frame(Esz, St) ->
    %{Eref,St}.
    luerl_heap:alloc_environment(Esz, St).

make_loc_frame(0) -> not_used;
make_loc_frame(Lsz) -> erlang:make_tuple(Lsz, nil).
...
do_while_loop(Is, Cont, Lvs, [Val | Stk], Env, Cs, St, Eis, Wis) ->
    case boolean_value(Val) of
        true ->
            emul(
                Wis,
                [Eis, [?WHILE_LOOP(Eis, Wis) | Is] | Cont],
                Lvs,
                Stk,
                Env,
                Cs,
...
do_numfor_loop(Is, Cont, Lvs, Stk, Env, Cs, St, N, Limit, Step, Fis) ->
    ...
    if
        Step > 0, N =< Limit ->
            emul(
                Fis,
                [[?NFOR_LOOP(N + Step, Limit, Step, Fis) | Is] | Cont],
                Lvs,
                [N | Stk],
                Env,
                Cs,
...
do_genfor_call(Is, Cont, Lvs, Stk, Env, Cs, St, Func, Data, Val, Fis) ->
    emul(
        [?FCALL, ?GFOR_LOOP(Func, Data, Fis) | Is],
        Cont,
        Lvs,
        [[Data, Val], Func | Stk],
        Env,
...
do_genfor_loop(Is, Cont, Lvs, [Vals | Stk], Env, Cs, St, Func, Data, Fis) ->
    case boolean_value(Vals) of
        true ->
            emul(
                Fis,
                [[?GFOR_CALL(Func, Data, hd(Vals), Fis) | Is] | Cont],
                Lvs,
                [Vals | Stk],
                Env,
                Cs,
...
build_tab_last(_, []) ->
    [];
build_tab_last(_, Last) ->
    error({boom, build_tab_acc, Last}).

build_tab_loop(0, Stk, Fs) -> {Fs, Stk};
build_tab_loop(C, [V, K | Stk], Fs) -> build_tab_loop(C - 1, Stk, [{K, V} | Fs]).
...
eq_op(_Op, A1, A2, St) when
    ?IS_TREF(A1), ?IS_TREF(A2); ?IS_USDREF(A1), ?IS_USDREF(A2)
->
...
lt_op(Op, A1, A2, St) -> op_meta(Op, A1, A2, <<"__lt">>, St).
...
concat_op(A1, A2, St) ->
    case luerl_lib:conv_list([A1, A2], [lua_string, lua_string]) of
        ...
        error -> op_meta('..', A1, A2, <<"__concat">>, St)
    end.
