...
alloc_environment(Fr, #luerl{heap=He0}=St)->
    %% io:format("ae: ~p\n~p\n~p\n", [Fr,St,He]),
    {Eref,He1} = alloc_environment_heap(Fr, He0),
    {Eref,St#luerl{heap=He1}}.

alloc_environment_heap(Fr, #heap{envtab=Et0,envfree=[N|Ns]}=He0) ->
    Et1 = ?SET_TABLE(N, Fr, Et0),
    He1 = He0#heap{envtab=Et1,envfree=Ns},
    {#eref{i=N},He1};
alloc_environment_heap(Fr, #heap{envtab=Et0,envfree=[],envnext=N}=He0) ->
    Et1 = ?SET_TABLE(N, Fr, Et0),
...
alloc_table(Itab, #luerl{heap=He0}=St) ->
    Tab = create_table(Itab),
    {Tref,He1} = alloc_table_heap(Tab, He0),
    {Tref,St#luerl{heap=He1}}.

alloc_table_heap(T, #heap{ttab=Ts0,tfree=[N|Ns]}=He0) ->
    %% io:fwrite("it1: ~p\n", [{N,T}]),
    Ts1 = ?SET_TABLE(N, T, Ts0),
    He1 = He0#heap{ttab=Ts1,tfree=Ns},
    {#tref{i=N},He1};
alloc_table_heap(T, #heap{ttab=Ts0,tfree=[],tnext=N}=He0) ->
    %% io:fwrite("it2: ~p\n", [{N,T}]),
    Ts1 = ?SET_TABLE(N, T, Ts0),
...
%% flat_set_table(Table, Key, Val, State) -> {Instr,State}.
%% flat_get_table_key(Table, Key, State) -> {Instrs,State)
...
flat_set_table_key(#tref{i=N}=Tab, Key, Val, #luerl{heap=He0}=St) ->
    Ts0 = He0#heap.ttab,
    #table{d=Dict0,meta=Meta}=T = ?GET_TABLE(N, Ts0),
    case ttdict:find(Key, Dict0) of
	{ok,_} ->				%Key exists
...
flat_set_table_int(#tref{i=N}=Tab, Key, I, Val, #luerl{heap=He0}=St) ->
    Ts0 = He0#heap.ttab,
    #table{a=Arr0,meta=Meta}=T = ?GET_TABLE(N, Ts0),
    case array:get(I, Arr0) of
	nil ->					%Key does not exist
...
flat_get_table_key(#tref{i=N}=T, Key, #luerl{heap=#heap{ttab=Ts}}=St) ->
    #table{d=Dict,meta=Meta} = ?GET_TABLE(N, Ts),
    case ttdict:find(Key, Dict) of
	{ok,Val} -> {value,Val,St};
	error ->
...
flat_get_table_int(#tref{i=N}=T, Key, I, #luerl{heap=#heap{ttab=Ts}}=St) ->
    #table{a=A,meta=Meta} = ?GET_TABLE(N, Ts),	%Get the table.
    case array:get(I, A) of
	nil ->
	    %% Key not present so try metamethod
...
flat_set_global_var(Var, Val, #luerl{heap=#heap{g=G}}=St) ->
    flat_set_table(G, Var, Val, St).

get_global_var(Var, #luerl{heap=#heap{g=G}}=St) ->
    get_table_key(G, Var, St).

flat_get_global_var(Var, #luerl{heap=#heap{g=G}}=St) ->
...
emul_1([?PUSH_GVAR(Key)|Is], Lvs, Stk, Env, St0) ->
    ...
    case flat_get_global_var(Key, St0) of
...
emul_1([?STORE_GVAR(Key)|Is], Lvs, [Val|Stk], Env, St0) ->
    case flat_set_global_var(Key, Val, St0) of
...
emul_1([?GET_LIT_KEY(Key)|Is], Lvs, [Tab|Stk], Env, St0) ->
    %% [?PUSH_LIT(K),?GET_KEY]
    %% {Val,St1} = get_table_key(Tab, Key, St0),
    %% emul(Is, Lvs, [Val|Stk], Env, St1);
    case flat_get_table(Tab, Key, St0) of
...
emul_1([?SET_LIT_KEY(Key)|Is], Lvs, [Tab,Val|Stk], Env, St) ->
    ...
    %% {Gis,St1} = flat_get_table(Tab, K, St0),
    %% emul(Gis ++ Is, Lvs, Stk, Env, St1);

    %% {Sis,St1} = flat_set_table(Tab, Key, Val, St0),
    %% emul(Sis ++ Is, Lvs, Stk, Env, St1);

emul_1([?NOP|Is], Lvs, Stk, Env, St) ->
    emul(Is, Lvs, Stk, Env, St);
emul_1([?FCALL(Func,Args)|Is], Lvs, Stk, Env, St) ->
...
emul_1([?AND_THEN(Then)|Is], Lvs, Stk, Env, St) ->
    do_and_then(Is, Lvs, Stk, Env, St, Then);
emul_1([?OR_ELSE(Else)|Is], Lvs, Stk, Env, St) ->
    do_or_else(Is, Lvs, Stk, Env, St, Else);
emul_1([?IF_TRUE(True)|Is], Lvs, Stk, Env, St) ->
    do_if_true(Is, Lvs, Stk, Env, St, True);
...
do_block(Is, Lvs, Stk, Env, St0, Lsz, Esz, Bis) ->
    L = make_loc_frame(Lsz),
    {Eref,St1} = make_env_frame(Esz, St0),
...
%% do_and_then(Instrs, LocalVars, Stack, Env, State, ThenInstrs) -> <emul>.
%% do_or_else(Instrs, LocalVars, Stack, Env, State, ElseInstrs) -> <emul>.

do_and_then(Is, Lvs, [Val|Stk1]=Stk0, Env, St, Then) ->
...
do_or_else(Is, Lvs, [Val|Stk1]=Stk0, Env, St, Else) ->
    %% This is an expression and must always leave a value on stack.
    case boolean_value(Val) of
	true ->
...
%% do_if(Instrs, LocalVars, Stack, Env, State, TrueInstrs) ->
%%     <emul>.
%%  Test value on stack to choose whether to do True instructions.

do_if_true(Is, Lvs, [Val|Stk], Env, St, True) ->
...
do_if(Is, Lvs0, [Val|Stk0], Env0, St0, True, False) ->
...
op('~', A, St) ->
    integer_op('~', A, St, <<"__bnot">>, fun (N) -> {ok,bnot(N)} end);
...
concat_op(A1, A2, St) ->
	...
	error ->
	    case get_metamethod(A1, A2, <<"__concat">>, St) of
		nil -> badarg_error('..', [A1,A2], St);
		Meth ->
		    {meta,Meth,[A1,A2],St}
	    end
    end.
