...
table() ->
    ...
    [
        ...
        {<<"loadstring">>, #erl_mfa{m = ?MODULE, f = loadstring}},
...
collectgarbage(_, [<<"collect">> | _], St) -> {[], luerl_heap:gc(St)};
%% {[],St};					%No-op for the moment

%Ignore everything else
collectgarbage(_, _, St) -> {[], St}.
...
ipairs(_, As, St) ->
    badarg_error(ipairs, As, St).

ipairs_next(_, [A], St) ->
    ipairs_next(nil, [A, 0], St);
...
rawlen(_, [#tref{} = T | _], St) -> {[luerl_lib_table:raw_length(T, St)], St};
...
tostring(_, [Arg | _], St) ->
    case luerl_heap:get_metamethod(Arg, <<"__tostring">>, St) of
        ...
        M when ?IS_FUNCTION(M) ->
            %Return {R,St1}
            luerl_emul:functioncall(M, [Arg], St)
    end;
tostring(_, As, St) ->
    badarg_error(tostring, As, St).
...
%Erlang functions
tostring(#erl_func{code = C}) ->
    iolist_to_binary([<<"function: ">>, io_lib:write(C)]);
%Erlang MFA triplets
tostring(#erl_mfa{m = M, f = F}) ->
...
%Functions defined in Lua
type(#funref{}) -> <<"function">>;
%Internal functions
type(#erl_func{}) -> <<"function">>;
...
do_setmetatable(#tref{} = Tref, Meta, St0) ->
    case luerl_heap:get_metamethod(Tref, <<"__metatable">>, St0) of
        nil ->
            ...
            {[Tref], St1};
        _ ->
            badarg_error(setmetatable, [Tref], St0)
    end.
