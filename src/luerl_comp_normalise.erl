...
functiondef(L, Name0, Ps0, B, St0) ->
    ...
    %Export Name1 and Ps1
    case is_method(Name0) of
        {yes, Name1} ->
            Ps1 = [{'NAME', L, <<"self">>} | Ps0];
        no ->
            Name1 = Name0,
...
flat_funcname({'.', _, L, R}, Rest) -> flat_funcname(L, [<<".">> | flat_funcname(R, Rest)]).
...
exp({nil, L}, St) ->
    {#lit{l = L, val = nil}, St};
exp({false, L}, St) ->
    {#lit{l = L, val = false}, St};
exp({true, L}, St) ->
    {#lit{l = L, val = true}, St};
exp({'NUMERAL', L, N}, St) ->
    {#lit{l = L, val = N}, St};
exp({'LITERALSTRING', L, S}, St) ->
    {#lit{l = L, val = S}, St};
exp({'...', L}, St) ->
    {var_name(L, '...'), St};
%% {#lit{l=L,v='...'},St};
