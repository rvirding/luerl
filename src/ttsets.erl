...
is_set(empty) -> true;
is_set({A, _, B}) -> is_set(A) andalso is_set(B);
is_set({A, _, B, _, C}) -> is_set(A) andalso is_set(B) andalso is_set(C);
is_set(_) -> false.
...
size(empty) -> 0;
size({A, _, B}) -> size(A) + size(B) + 1;
size({A, _, B, _, C}) -> size(A) + size(B) + size(C) + 2.
...
to_list({A, X, B}, Tail) -> to_list(A, [X | to_list(B, Tail)]);
to_list({A, X, B, Y, C}, Tail) -> to_list(A, [X | to_list(B, [Y | to_list(C, Tail)])]).
...
%1.1
del_up2_l({up, L}, X, {M, Y, R}) ->
    {up, {L, X, M, Y, R}};
%2.1
del_up2_l({up, A}, X, {B, Y, C, Z, D}) ->
    {{A, X, B}, Y, {C, Z, D}};
del_up2_l(L, X, R) ->
    {L, X, R}.

%1.2
del_up2_r({L, X, M}, Y, {up, R}) ->
    {up, {L, X, M, Y, R}};
%2.2
del_up2_r({A, X, B, Y, C}, Z, {up, D}) ->
    {{A, X, B}, Y, {C, Z, D}};
del_up2_r(L, X, R) ->
    {L, X, R}.
...
%3a.1
del_up3_l({up, A}, X, {B, Y, C}, Z, D) ->
    {{A, X, B, Y, C}, Z, D};
%4a.1
del_up3_l({up, A}, W, {B, X, C, Y, D}, Z, E) ->
    {{A, W, B}, X, {C, Y, D}, Z, E};
del_up3_l(A, X, B, Y, C) ->
    {A, X, B, Y, C}.

%3a.2
del_up3_m({A, X, B}, Y, {up, C}, Z, D) ->
    {{A, X, B, Y, C}, Z, D};
%3b.1
del_up3_m(A, X, {up, B}, Y, {C, Z, D}) ->
    {A, X, {B, Y, C, Z, D}};
%4a.2
del_up3_m({A, W, B, X, C}, Y, {up, D}, Z, E) ->
    {{A, W, B}, X, {C, Y, D}, Z, E};
%4b.1
del_up3_m(A, W, {up, B}, X, {C, Y, D, Z, E}) ->
    {A, W, {B, X, C}, Y, {D, Z, E}};
del_up3_m(A, X, B, Y, C) ->
    {A, X, B, Y, C}.

%3b.2
del_up3_r(A, X, {B, Y, C}, Z, {up, D}) ->
    {A, X, {B, Y, C, Z, D}};
%4b.2
del_up3_r(A, W, {B, X, C, Y, D}, Z, {up, E}) ->
    {A, W, {B, X, C}, Y, {D, Z, E}};
del_up3_r(A, X, B, Y, C) ->
    {A, X, B, Y, C}.
...
intersection([S]) -> S;
intersection([S | Ss]) -> lists:foldl(fun(S1, I) -> intersection(S1, I) end, S, Ss).
