...
%Remove unnecessary ?PUSH_VALS
assign_loop([V], [E], St0) ->
    %Last argument to one variable
    {Ie, St1} = exp(E, single, St0),
    {Iv, St2} = assign_prefixexp(V, St1),
    {Ie ++ Iv, St2};
assign_loop([V | Vs], [E], St0) ->
    %Last argument to rest of vars
    {Ie, St1} = exp(E, multiple, St0),
...
assign_loop_exp([E | Es], St0) ->
    %It will be dropped anyway
    {Ie, St1} = exp(E, single, St0),
    {Ias, St2} = assign_loop_exp(Es, St1),
    %Pop unneeded value off stack
    {Ie ++ Ias ++ [?POP], St2};
assign_loop_exp([], St) ->
    {[], St}.
...
assign_prefixexp_rest(#dot{exp = Exp, rest = Rest}, St0) ->
    ...
    {Ir, St2} = assign_prefixexp_rest(Rest, St1),
    {Ie ++ Ir, St2};
assign_prefixexp_rest(Exp, St) ->
    assign_prefixexp_element(Exp, St).
...
%Remove unnecessary ?PUSH_VALS
assign_local_loop([V], [E], St0) ->
    %Last argument to one variable!
    {Ie, St1} = exp(E, single, St0),
    {Ie ++ set_var(V), St1};
assign_local_loop([V | Vs], [E], St0) ->
    %Last argument to many vars!
    {Ie, St1} = exp(E, multiple, St0),
...
exp(#op{op = 'and', args = [A1, A2]}, S, St0) ->
    ...
    %Must handle single/multiple
    {Ia1 ++ [?AND_THEN(Ia2)], St2};
...
prefixexp_element(#fcall{args = As}, S, St0) ->
    ...
    %Function call returns list
    {single_value(S, Ifs), St1};
...
prefixexp_element(#mcall{meth = #lit{val = K}, args = As}, S, St0) ->
    ...
    %Method call returns list
    {single_value(S, Ims), St1}.
...
functiondef(
    #fdef{l = Anno0, pars = Ps0, body = Ss, lsz = Lsz, esz = Esz},
...
%Tail is index for varargs
func_pars([#evar{n = '...', i = I}]) -> -I;
