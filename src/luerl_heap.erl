...
create_table(Itab) ->
    D0 = ttdict:new(),
    %Arrays with 'nil' as default
    A0 = array:new([{default, nil}]),
    %Ignore nil values
    Init = fun
        ({_, nil}, {D, A}) ->
            {D, A};
...
set_table_key_key(#tref{i = N} = Tab, Key, Val, #luerl{tabs = Tst0} = St) ->
    ...
    case ttdict:find(Key, Dict0) of
        ...
        {ok, _} ->
            Dict1 =
                if
                    Val =:= nil -> ttdict:erase(Key, Dict0);
...
%Just find the metamethod
get_table_key(Other, Key, St) ->
...
get_table_key_int(#tref{i = N} = T, Key, I, #luerl{tabs = #tstruct{data = Ts}} = St) ->
    %Get the table.
    #table{a = A, meta = Meta} = ?GET_TABLE(N, Ts),
...
raw_get_table_key(#tref{i = N}, Key, #luerl{tabs = Tst}) when
    is_integer(Key), Key >= 1
->
...
raw_set_table_key(#tref{} = Tref, Key, Val, #luerl{tabs = Tst0} = St) when
    is_integer(Key), Key >= 1
->
...
%Other types have no metatables
get_metamethod_tab(_, _, _) ->
    nil.
...
get_metatable(nil, #luerl{meta = Meta}) ->
    Meta#meta.nil;
...
%Gc table info table, seen
-record(gct, {t, s}).

gc(
    #luerl{
        tabs = #tstruct{data = Tt0, free = Tf0} = Tab0,
...
%% The call stack.
mark(
    [#call_frame{func = Funref, lvs = Lvs, env = Env} | Todo],
    More0,
    GcT,
    GcE,
    GcU,
