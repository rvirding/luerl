...
%% luerl_old:eval(String|Binary|Form, State) -> Result.
...
%% luerl_old:evalfile(Path, State) -> {ok, Result} | {error,Reason}.
...
%% luerl_old:do(String|Binary|Form, State) -> {Result, NewState}
...
%% luerl_old:dofile(Path, State) -> {Result, NewState}.
...
path_loadfile(Name, St) ->
    Path =
        case os:getenv("LUA_LOAD_PATH") of
...
do_stackframe(#call_frame{func = Funref, args = Args}, {Line, Trace}, St) ->
    case Funref of
        #funref{} ->
            ...
            Name =
                case luerl_anno:get(name, Anno) of
                    undefined -> <<"-no-name-">>;
                    N -> N
                end,
...
do_stackframe(#current_line{line = Line}, {_, Trace}, _St) ->
    {Line, Trace};
%Ignore these
do_stackframe(#loop_frame{}, Acc, _St) ->
    Acc.
...
decode(#funref{} = Fun, State, _) ->
    ...
    %Just a bare fun
    F;
decode(#erl_func{code = Fun}, _, _) ->
    Fun;
decode(#erl_mfa{m = M, f = F, a = A}, _, _) ->
    {M, F, A};
