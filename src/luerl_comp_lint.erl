...
stmt(#local_assign_stmt{} = For, St) -> local_assign_stmt(For, St);
stmt(#local_fdef_stmt{} = For, St) -> local_fdef_stmt(For, St);
stmt(#expr_stmt{} = E, St) -> expr_stmt(E, St).
...
exp(#op{args = Es}, St) -> explist(Es, St);
exp(#tabcon{fields = Fs}, St) -> tableconstructor(Fs, St);
exp(E, St) -> prefixexp(E, St).
...
var(#var{l = Anno, name = '...'}, St) ->
    case lists:keymember('...', #var.name, St#lint.pars) of
        true -> St;
        false -> illegal_varargs_error(Anno, St)
    end;
var(_Var, St) ->
    St.
