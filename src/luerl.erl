...
encode(true, St) ->
    {true, St};
encode(B, St) when is_binary(B) ->
    {B, St};
encode(A, St) when is_atom(A) ->
    {atom_to_binary(A, latin1), St};
%Integers and floats
encode(N, St) when is_number(N) ->
    {N, St};
encode(F, St) when ?IS_MAP(F) ->
    encode(maps:to_list(F), St);
