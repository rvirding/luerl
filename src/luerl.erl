...
decode(#tref{} = T, St, In) -> decode_table(T, St, In);
decode(#usdref{} = U, St, In) -> decode_userdata(U, St, In);
decode(#funref{} = Fun, St, In) -> decode_luafunc(Fun, St, In);
decode(#erl_func{} = Fun, St, In) -> decode_erlfunc(Fun, St, In);
decode(#erl_mfa{} = Mfa, St, In) -> decode_erlmfa(Mfa, St, In);
%Shouldn't have anything else
decode(Lua, _, _) -> error({badarg, Lua}).
