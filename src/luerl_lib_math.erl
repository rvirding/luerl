...
%Naughty, copied from source
-define(RAND_SEED(S1, S2, S3),
    %PRIME1
    {
        (abs(S1) rem (30269 - 1) + 1),
        %PRIME2
        (abs(S2) rem (30307 - 1) + 1),
        %PRIME3
        (abs(S3) rem (30323 - 1) + 1)
    }
).
% random has just three integers for state so no special work needed.
-define(RAND_EXTERNALIZE(S), S).
-define(RAND_INTERNALIZE(S), S).
-endif.

install(St0) ->
    %Default initial random seed
    St1 = St0#luerl{rand = ?RAND_SEED()},
...
table() ->
    [
        ...
        %For 5.2 backwards compatibility
        {<<"sinh">>, #erl_mfa{m = ?MODULE, f = sinh}},
        {<<"sqrt">>, #erl_mfa{m = ?MODULE, f = sqrt}},
        {<<"tan">>, #erl_mfa{m = ?MODULE, f = tan}},
        %For 5.2 backwards compatibility
        {<<"tanh">>, #erl_mfa{m = ?MODULE, f = tanh}},
...
ldexp(_, As, St) ->
    case get_number_args(As) of
        ...
        %% 	    {[X],St};
        _ ->
            badarg_error(ldexp, As, St)
    end.

log(_, As, St) ->
    case get_number_args(As) of
        [N1, N2 | _] when is_number(N1), N2 == 10 ->
            %Seeing it is builtin
            {[math:log10(N1)], St};
        [N1, N2 | _] when is_number(N1), is_number(N2) ->
...
log10(_, As, St) ->
    case get_number_args(As) of
        %Bit hacky
        [N | _] when N == 0 -> {[-500.0], St};
        [N | _] when is_number(N) ->
            {[math:log10(N)], St};
        _ ->
            badarg_error(log10, As, St)
    end.
