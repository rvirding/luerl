...
do_byte_ij(S, _, I, J) -> [N || N <- binary_to_list(S, I, J)].
...
format(_, As, St) ->
    badarg_error(format, As, St).

%To keep dialyzer quiet
-spec gmatch(_, [_], _) -> no_return().
...
do_gsub(As, St) ->
    case luerl_lib:conv_list(As, [lua_string, lua_string, lua_any, lua_integer]) of
        ...
        %'all' bigger than any number
        [S, P, R] ->
            do_gsub(S, byte_size(S), P, R, all, St);
        _ ->
            throw({error, {badarg, gsub, As}, St})
    end.
...
%It can still match at end!
gsub_match_loop(<<>>, _, Pat, I, _, _) ->
...
gsub_repl_str(Cas, S, [$%, $0 | R]) ->
    %Force to string!
    Cstr = luerl_lib:arg_to_string(match_cap(hd(Cas), S)),
...
gsub_repl_str(Cas, S, [$%, C | R]) when C >= $1, C =< $9 ->
    case lists:keysearch(C - $0, 1, Cas) of
        {value, Ca} ->
            ...
            [Cstr | gsub_repl_str(Cas, S, R)];
        false ->
            throw({error, {illegal_index, capture, C - $0}})
    end;
...
%It can still match at end!
match_loop(S, L, Pat, I) when I > L ->
    case match_pat(S, Pat, I) of
        {match, Cas, _, _} -> Cas;
        %Now we haven't found it
        nomatch -> []
    end;
...
%Capture position
match_cap({_, P, Len}, _, I) when Len < 0 ->
    P + I - 1;
%Capture
match_cap({_, P, Len}, S, I) ->
    %Binaries count from 0
    binary_part(S, P + I - 2, Len).
...
sub(_, As, St) ->
    case luerl_lib:conv_list(As, [lua_string, lua_integer, lua_integer]) of
        [S, I | Js] ->
            Len = byte_size(S),
            %Just I, or both I and J
            Sub = do_sub(S, Len, I, Js),
            {[Sub], St};
        %nil, [_] or []
        _ ->
            badarg_error(sub, As, St)
    end.
...
do_sub_m(Len, I) when I < 0 -> Len + I + 1;
do_sub_m(_, I) -> I.
...
match_pat(['^' | Ps] = Ps0, Cs, I, Ca, Cas) ->
    if
        I =:= 1 -> match_pat(Ps, Cs, 1, Ca, Cas);
...
match_pat([{kclosure, P} = K | Ps], Cs, I, Ca, Cas) ->
    ...
    %First try with it
    case match_pat([P, K | Ps], Cs, I, Ca, Cas) of
        {match, _, _, _, _} = M -> M;
        %Else try without it
        {nomatch, _, _, _, _, _} -> match_pat(Ps, Cs, I, Ca, Cas)
    end;
%The easy way
match_pat([{pclosure, P} | Ps], Cs, I, Ca, Cas) ->
    match_pat([P, {kclosure, P} | Ps], Cs, I, Ca, Cas);
match_pat([{mclosure, P} = K | Ps], Cs, I, Ca, Cas) ->
    %First try without it
    case match_pat(Ps, Cs, I, Ca, Cas) of
        {match, _, _, _, _} = M -> M;
        %Else try with it
        {nomatch, _, _, _, _, _} -> match_pat([P, K | Ps], Cs, I, Ca, Cas)
    end;
match_pat([{optional, P} | Ps], Cs, I, Ca, Cas) ->
    %First try with it
    case match_pat([P | Ps], Cs, I, Ca, Cas) of
        {match, _, _, _, _} = M -> M;
        %Else try without it
        {nomatch, _, _, _, _, _} -> match_pat(Ps, Cs, I, Ca, Cas)
    end;
...
%Matches anything
match_pat(['.' | Ps], <<_, Cs/binary>>, I, Ca, Cas) ->
...
balance(<<L, Cs/binary>>, I, L, R, D) ->
    balance(Cs, I + 1, L, R, D + 1);
balance(<<_, Cs/binary>>, I, L, R, D) ->
    balance(Cs, I + 1, L, R, D);
balance(<<>>, _, _, _, _) ->
    error.
...
match_char_set([C | _], C) ->
    true;
match_char_set([_ | Set], C) ->
    match_char_set(Set, C);
match_char_set([], _) ->
    false.
...
is_c_char(C) when C >= 0, C =< 31 -> true;
is_c_char(C) when C >= 128, C =< 159 -> true;
is_c_char(_) -> false.

%All digits
is_d_char(C) -> (C >= $0) and (C =< $9).

%All printable characters
is_g_char(C) when C >= 33, C =< 126 -> true;
is_g_char(C) when C >= 161, C =< 255 -> true;
is_g_char(_) -> false.

%All lowercase letters
is_l_char(C) when C >= $a, C =< $z -> true;
is_l_char(C) when C >= 224, C =< 246 -> true;
is_l_char(C) when C >= 248, C =< 255 -> true;
is_l_char(_) -> false.

%All punctutation characters
is_p_char(C) when C >= 33, C =< 47 -> true;
...
%Space characters
is_s_char(C) when C >= 9, C =< 13 -> true;
is_s_char(32) -> true;
is_s_char(160) -> true;
is_s_char(_) -> false.

%All uppercase letters
is_u_char(C) when C >= $A, C =< $Z -> true;
