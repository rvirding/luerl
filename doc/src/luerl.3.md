...
### **`luerl:call_function_enc(KeyPath, Args, State) -> {ok, Result, State} | {lua_error, Error, State}`**
...
Here we are going to look at passing in command strings into the ``luerl:do/2`` function, especially when these strings contain Lua strings. The problem is to make sure that the strings in the Lua commands are processes correctly.

First just just doing it from the standard Lua shell as the source of truth:
...
Now doing it from Elixir using the ``~C`` sigil to get the handling of
the string right:
...
So it works as expected. We use ``io:write`` to write out the
characters in the binary without any parsing or trying to be smart.
