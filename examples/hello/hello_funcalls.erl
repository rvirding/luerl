...
run() ->
    ...
    % The argument part of {M,F,A} won't get encoded, so that is why
    % we can get away with passing a tuple here.
    {ok,Lua2} = luerl:set_table_keys_dec([<<"hello_funcall">>],
                                         {hello_funcalls,mfa_function,{1,2}},
                                         Lua),

    {ok,Res1, _} = luerl:do(LuaScript, Lua1),
    io:format("regular_function got ~p~n", Res1),

    {ok,Res2, _} = luerl:do(LuaScript, Lua2),
