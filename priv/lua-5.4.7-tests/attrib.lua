...
print("NOT requiring coroutine -- unimplemented")
--assert(require"coroutine" == coroutine)

assert(type(package.path) == "string")
print("NOT checking type of package.cpath")
...
print("testing package.searchpath error message generation")
do
  ...
  print("NOT testing path with max templates")
  --assert(not s and
  --       string.find(err, string.rep("xuxu", 10)) and
  --       #string.gsub(err, "[^\n]", "") >= max)
  print("testing path with one very long template")
...
print("NOT testing error messages when package.path is empty")
-- TODO: 
-- ** exception error: bad argument
--   in function  binary:split/3
--      called as binary:split({tref,15},<<";">>,[global])
--      *** argument 1: not a binary
--do
--  local oldpath = package.path
--  package.path = {}
--  local s, err = pcall(require, "no-such-file") -- error here
--  assert(not s and string.find(err, "package.path"))
--  package.path = oldpath
--end

print("NOT testing 'require' message")
-- TODO: Luerl error messages probably don't match the upstream Lua messages. 
...
print("NOT testing 'require' for specific directories/libraries")
--if not _port then --[
--
...
print("NOT testing table constructors with nil key")
...
print("NOT testing _ENV constant declaration with invalid type")
--do
--  -- _ENV constant
--  local function foo ()
--    local _ENV <const> = 11
...
print("NOT testing conflicts in multiple assignment")
-- TODO: {illegal_index, 1, 1}
--do
--  local a,i,j,b
--  a = {'a', 'b'}; i=1; j=2; b=a
--  i, a[i], a, j, a[j], a[i+j] = j, i, i, b, j, i  -- error here
...
print("NOT repeating test with upvalues")
--do
--  local a,i,j,b
--  a = {'a', 'b'}; i=1; j=2; b=a
--  local function foo ()
